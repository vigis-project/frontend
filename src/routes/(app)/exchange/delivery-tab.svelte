	...
	<div class="flex h-[400px] w-1/2 flex-col pr-4">
		...
		<div class:mb-6={!cityEmptyError && !cityValidationError}>
			<span class="block text-sm font-medium"
				>Город <span class="font-bold text-red-500">*</span></span
			>
			<input
				type="text"
				placeholder="Москва"
			...
			{#if cityValidationError && !cityEmptyError}
				<span class="text-sm text-red-500"
					>Некорректное имя. Только кириллица, до 30 символов.</span
				>
			{/if}
		</div>
		...
		<div class:mb-6={!streetEmptyError && !streetValidationError}>
			<span class="block text-sm font-medium"
				>Улица <span class="font-bold text-red-500">*</span></span
			>
			<input
				type="text"
		...
		<div class="flex">
			<div class="mr-2 w-1/3" class:mb-6={!buildingEmptyError && !buildingValidationError}>
				<span class="block text-sm font-medium"
					>Номер строение <span class="font-bold text-red-500">*</span></span
				>
				<input
					type="text"
			...
			<div class="mr-2 w-1/3" class:mb-6={!householdEmptyError && !householdValidationError}>
				<span class="block text-sm font-medium"
					>Номер дома <span class="font-bold text-red-500">*</span></span
				>
				<input
					type="text"
					autocomplete="street-address"
			...
			<div class="mr-2 w-1/3" class:mb-6={!flatEmptyError && !flatValidationError}>
		...
		<div class:mb-6={!indexEmptyError && !indexValidationError}>
			<span class="block text-sm font-medium"
				>Индекс <span class="font-bold text-red-500">*</span></span
			>
			<input
				type="text"
				aria-label="XXXYYY"
			...
			{#if indexValidationError && !indexEmptyError}
				<span class="text-sm text-red-500"
					>Некорректный формат. Индекс должен состоять из 6 цифр.</span
				>
			{/if}
		</div>
		<p class="pt-5">
	...
	<div class="flex h-[400px] w-1/2 flex-col pl-4">
		...
		<div class:mb-6={!lastNameEmptyError && !lastNameValidationError}>
			<span class="text-sm font-medium"
				>Фамилия <span class="font-bold text-red-500">*</span></span
			>
			<input
				type="text"
			...
			{#if lastNameValidationError && !lastNameEmptyError}
				<span class="text-sm text-red-500"
					>Некорректная фамилия. Только кириллица, до 50 символов.</span
				>
			{/if}
		</div>
		...
		<div class:mb-6={!firstNameEmptyError && !firstNameValidationError}>
			<span class="text-sm font-medium"
				>Имя <span class="font-bold text-red-500">*</span></span
			>
			<input
				type="text"
				autocomplete="name"
			...
			{#if firstNameValidationError && !firstNameEmptyError}
				<span class="text-sm text-red-500"
					>Некорректное имя. Только кириллица, до 25 символов.</span
				>
			{/if}
		</div>
		...
		<div class:mb-6={!surNameError}>
			...
			{#if surNameError}
				<span class="text-sm text-red-500"
					>Некорректное отчество. Только кириллица, до 25 символов.</span
				>
			{/if}
		</div>
	</div>
...
<div class="mt-4 flex justify-between">
	...
	<button onclick={confirmData} class="rounded bg-blue-500 p-2 px-6 text-white"
