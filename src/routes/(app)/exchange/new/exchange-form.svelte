...
<form class="mx-auto w-full max-w-6xl">
	<Card.Root class="bg-walnut-muted/80 border-walnut-muted w-full rounded-lg border text-white">
		<Tabs.Root bind:value={currentTab} class="w-full">
			<Tabs.List
				class="bg-walnut-muted text-muted-foreground grid w-full grid-cols-3 rounded-t-md"
			>
				<Tabs.Trigger value="exchange" class="rounded-none" disabled>
					Хочу обменять
				</Tabs.Trigger>
				<Tabs.Trigger value="receive" class="rounded-none" disabled
					>Хочу получить</Tabs.Trigger
				>
			...
			<Card.Content>
				<Tabs.Content value="exchange">
					<ExchangeTab
						...
						bind:year
						bind:title
						bind:firstName={authorFirstName}
						bind:lastName={authorLastName}
					/>
				</Tabs.Content>
